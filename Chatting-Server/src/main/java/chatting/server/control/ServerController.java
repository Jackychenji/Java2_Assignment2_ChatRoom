package chatting.server.control;
...
import static chatting.share.model.Message.CHAT;
import static chatting.share.model.Message.LOGOUT;
...
//        sendBroadCast(msg);
  ...
  private void sendBroadCast(Message msg) {
    ...
    for (Map.Entry<String, UserInfo> entry : s) {
      if (msg.getTarger_user().size() == 0) {
        send(entry.getValue(), msg);
      } else {
        if (msg.getTarger_user().contains(entry.getKey())) {
          send(entry.getValue(), msg);
        }
      }
    }
